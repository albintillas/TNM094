...
struct rendering::Window::Impl {
    ...
    // Add tracking for delta time and FPS
...
Window::Impl::Impl(std::string_view title, int width, int height)
...
void Window::beginFrame() {
    ...
    impl->deltaTime = static_cast<float>(currentTime - impl->lastFrameTime);
    impl->lastFrameTime = currentTime;
    
    // Update FPS counter
    impl->frameCount++;
    impl->fpsUpdateTimer += impl->deltaTime;
    if (impl->fpsUpdateTimer >= 0.5) { // Update FPS every half second
        impl->fps = impl->frameCount / impl->fpsUpdateTimer;
        impl->frameCount = 0;
        impl->fpsUpdateTimer = 0.0;
    }
...
void Window::drawPoints(std::span<const glm::vec2> pos, std::span<const float> radius,
    ...
    const size_t count = std::min({pos.size(), radius.size(), color.size()});
    drawPoints(pos.data(), radius.data(), color.data(), count);
}

void Window::drawPoints(const std::vector<glm::vec2>& pos, const std::vector<float>& radius,
